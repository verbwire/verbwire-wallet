...
export class VerbwireWallet extends EventEmitter {
    ...
    openIframe() {
        const overlay = document.querySelector<HTMLElement>('.iframe-overlay');
        if (overlay) {
            // Show the overlay
    ...
    resizeIframe(height) {
        ...
        const iframe = document.querySelector('.wallet-iframe') as HTMLElement;
...
`;
    ...
    signMessage(data: {message?: string} = {}) {
        const { message } = data;
    ...
    sendTransaction(options: {to?: string, value?: any, data?: any, from?: string} = {}) {
