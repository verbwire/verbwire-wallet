...
import { VerbwireWallet } from './VerbwireWallet';
import EventEmitter from "eventemitter3";



export class VerbwireWalletConnector extends Connector {
    ...
    constructor({chains, options}) {
        ...
        if (!this.provider) {
            ...
            this.provider = new VerbwireWallet(this.applicationId, this.network, this.onLogin);
    ...
    async updateChainIdInLocalStorage(chainId: any) {
        if (typeof window !== 'undefined') {
    ...
    async connect(config?: { chainId?: number | undefined; }): Promise<Required<ConnectorData>> {
    ...
    async networkNameToChainId(networkName) {
        ...
        if (typeof networkName === 'number') return networkName;
